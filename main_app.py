...
def save_cached_session():
    cache = {
        ...
        "prefetched": st.session_state.get("prefetched", {}),
    }
...
if 'words_due' not in st.session_state:
    ...
    if cached_state:
        ...
        with st.spinner("Building your session..."):
            ...
            # Generate the whole session up front in one or two API calls
            # instead of one call per card. Anything this doesn't cover is
            # generated on demand exactly as before, so a partial or empty
            # result is safe.
            with st.spinner("Preparing your session..."):
                try:
                    st.session_state.prefetched = prefetch_session_exercises(
                        st.session_state.words_due,
...
def _attempt_generate_exercise():
    ...
    """
    # Batch prefetch cache: the whole session is generated up front in one
    # or two API calls. A hit here costs ZERO calls. A miss falls straight
    # through to the original per-card path below, so this can only help.
    idx_str = str(st.session_state.current_index)
    prefetched = st.session_state.get("prefetched") or {}
    cached = prefetched.pop(idx_str, None)
    if cached:
        st.session_state.prefetched = prefetched
        return cached

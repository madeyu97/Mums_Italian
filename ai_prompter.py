...
def _call_llm_json(prompt: str, use_json_mode: bool = True, max_tokens: int = None):
    """
    ...
    kwargs = {
        ...
        "max_completion_tokens": max_tokens or MAX_GEN_TOKENS,
    }
...
{lines}
    ...
    TRANSLATION FAITHFULNESS (CRITICAL — most common failure):
    "english_correct" must be a COMPLETE, EXACT translation of "italian".
    Every element in one MUST appear in the other. In particular, do NOT
    put a time, place, weather or manner detail in one and omit it from
    the other. Check word by word before returning.
      WRONG: italian "Stasera la città è bella"
             english "The city is beautiful"          (drops "stasera")
      RIGHT: italian "Stasera la città è bella"
             english "This evening the city is beautiful"
      WRONG: italian "Leggevo un libro"
             english "I was reading a book while it was raining"
                                                       (invents the rain)
      RIGHT: italian "Leggevo un libro mentre pioveva"
             english "I was reading a book while it was raining"
    If in doubt, use a SHORTER Italian sentence you can translate exactly.

    Output a raw JSON object EXACTLY in this format (no prose, no markdown):
    {{
        "italian": "<the sentence with proper accents>",
        "english_correct": "<COMPLETE translation — every Italian element present, nothing invented>",
    ...
    use_json_mode = True
    # 2 attempts, not 3. Attempt 1 uses Groq's JSON mode; attempt 2 falls
    # back to client-side parsing, which covers the realistic failure.
    for attempt in range(2):
        try:
            ...
            logging.warning(
                f"Dictation generation attempt {attempt + 1}/2 failed "
...
def _generate_conjugation_drill_once(infinitive, english, tense, person, attempt=0, prior_error=None):
    ...
    last_error_desc = None
    use_json_mode = True
    # 2 sub-attempts, not 3. Combined with the 2 outer verifier attempts
    # this caps a conjugation card at 4 API calls instead of 6 — the retry
    # multiplication was a major source of rate-limit pressure.
    for sub_attempt in range(2):
        try:
            ...
            last_error_category, last_error_desc = category, desc
            logging.warning(
                f"Conjugation drill API attempt {sub_attempt + 1}/2 failed "
                f"[{category}] (json_mode={use_json_mode}): {desc}"
            )
            if category == "auth":
...
# BATCH GENERATION
# ======================================================================
# WHY: the app previously made one LLM call PER CARD (plus retries), which
# is why it hit Groq's free-tier limits when simpler apps don't. These
# functions generate a whole session's cards in one or two calls.
#
# SAFETY CONTRACT: batching is a pure optimisation layer. Every function
# here returns a list aligned to its input with None for anything that
# failed, and never raises. Any card the batch couldn't produce simply
# falls through to the existing per-card generator. Worst case, behaviour
# is identical to before batching existed.

BATCH_MAX_ITEMS = 6      # cap per call so the JSON can't run past the token limit
BATCH_MAX_TOKENS = 8192  # generous ceiling for a multi-card payload
...
def _finalise_batch_dictation_item(raw_item, target_word_dict):
    """
    Convert one raw batch item into the same dict shape a single-card
    generation returns, applying the same agreement verifier and
    distractor filtering. Returns None if the item is unusable.
    """
    try:
        italian = (raw_item.get("italian") or "").strip()
        english = (raw_item.get("english_correct") or "").strip()
        if not italian or not english:
            return None

        distractors = [
            d.strip() for d in (raw_item.get("english_distractors") or [])
            if isinstance(d, str) and d.strip()
        ]

        # Same subject/verb agreement check the single-card path uses.
        ok, allowed_subjects, detected_verb = _verify_subject_agreement(italian, english)
        if not ok:
            logging.warning(
                f"[batch] Verb/subject mismatch: '{detected_verb}' needs {allowed_subjects}, "
                f"got {english!r} — item dropped, will regenerate individually."
            )
            return None

        # Drop distractors carrying the same person mismatch.
        if allowed_subjects:
            distractors = [
        ...
        # Need at least 3 usable distractors for the listening MCQ.
        if len(distractors) < 3:
            return None
        distractors = distractors[:3]

        word_breakdown = []
        for item in (raw_item.get("word_breakdown") or []):
        ...
        return {
            ...
            "grammar_point": gp,
            "expression_note": {},
            "target_category": _classify_target(
                target_word_dict.get("italian", ""),
                target_word_dict.get("english", ""),
                target_word_dict.get("hint", "") or "",
            ),
        }
    except Exception as e:
        logging.warning(f"[batch] Could not finalise dictation item: {e}")
        return None


def _build_dictation_batch_prompt(word_dicts):
    targets = []
    for i, w in enumerate(word_dicts, 1):
        hint = (w.get("hint") or "").strip()
        hint_part = f" [hint: {hint}]" if hint else ""
        targets.append(
            f'{i}. "{w.get("italian","")}" (means: {w.get("english","")}){hint_part}'
        )
    target_block = "\n".join(targets)

    return (
        "You are an expert Italian tutor. Produce ONE listening exercise for "
        "EACH numbered target word below.\n\n"
        "TARGET WORDS:\n" + target_block + "\n\n"
        "For each target write:\n"
        "  - a short, natural Italian sentence (4-10 words) that uses that "
        "target word, with correct accents and elision\n"
        "  - a COMPLETE English translation of that sentence\n"
        "  - exactly 3 wrong-but-plausible English options (same length and "
        "register as the correct one; vary the target word's grammar or "
        "meaning, not random words)\n"
        "  - a short word-by-word breakdown\n"
        "  - a one-line grammar note\n\n"
        "TRANSLATION FAITHFULNESS (most common failure — check every item):\n"
        "english_correct must be an EXACT, COMPLETE translation of italian. "
        "Every element in one must appear in the other. Never put a time, "
        "place, weather or manner detail in one and omit it from the other.\n"
        '  WRONG: "Stasera la citta e bella" / "The city is beautiful"  '
        "(drops stasera)\n"
        '  RIGHT: "Stasera la citta e bella" / "This evening the city is beautiful"\n'
        "If unsure, write a SHORTER Italian sentence you can translate exactly.\n\n"
        "SUBJECT-VERB AGREEMENT: Italian drops subject pronouns, so the verb "
        "ending fixes the English subject. sono=I/They, sei=You, e/ha=He/She/It, "
        "siamo=We, siete=You all, hanno=They; -o=I, -i=You, -a/-e=He/She, "
        "-iamo=We, -ate/-ete/-ite=You all, -ano/-ono=They. "
        '"sono" is NEVER "He/She is". "e" is NEVER "I am". '
        "Re-check each english_correct's subject against its verb before returning.\n\n"
        "Return ONLY this JSON object, with one entry per target, in order:\n"
        '{"cards":[{"index":1,"italian":"...","english_correct":"...",'
        '"english_distractors":["...","...","..."],'
        '"word_breakdown":[{"italian":"...","english":"...","note":"..."}],'
        '"grammar_point":{"structure":"...","explanation":"..."}}]}'
    )


def generate_dictation_batch(word_dicts):
    """
    Generate dictation-style cards for many words in ONE API call.

    Returns a list the same length as word_dicts, with None wherever an
    item could not be produced or failed verification. Never raises.
    """
    results = [None] * len(word_dicts)
    if not word_dicts:
        return results

    for chunk_start in range(0, len(word_dicts), BATCH_MAX_ITEMS):
        chunk = word_dicts[chunk_start:chunk_start + BATCH_MAX_ITEMS]
        prompt = _build_dictation_batch_prompt(chunk)
        ...
        for attempt in range(2):
            try:
                data = _call_llm_json(
                    prompt, use_json_mode=use_json_mode, max_tokens=BATCH_MAX_TOKENS
                )
                break
            except Exception as e:
                category, desc = _classify_llm_error(e)
                logging.warning(
                    f"[batch] Dictation batch attempt {attempt + 1}/2 failed "
                    f"[{category}]: {desc}"
                )
                _record_last_error(category)
                if category in ("auth", "rate_limit", "model_gone"):
                    break
                if category in ("json_parse", "truncated"):
                    use_json_mode = False
                if attempt < 1:
                    _wait_before_retry(category, attempt)

        if not isinstance(data, dict):
            continue  # whole chunk falls back to per-card generation

        cards = data.get("cards")
        if not isinstance(cards, list):
            continue

        # Match by the model's 1-based index, falling back to position.
        by_index = {}
        ...
        for local_i, word in enumerate(chunk):
            raw_item = by_index.get(local_i + 1)
            if raw_item is None:
                continue
            finalised = _finalise_batch_dictation_item(raw_item, word)
            if finalised is not None:
                results[chunk_start + local_i] = finalised

    produced = sum(1 for r in results if r is not None)
    logging.info(f"[batch] Dictation batch produced {produced}/{len(word_dicts)} cards.")
    if produced:
        _record_last_error(None)
    return results


def _build_conjugation_batch_prompt(specs):
    """specs: list of (infinitive, english, tense, person)."""
    lines = []
    for i, (inf, eng, tense, person) in enumerate(specs, 1):
        lines.append(f'{i}. verb "{inf}" ({eng}) — tense: {tense}, person: {person}')
    target_block = "\n".join(lines)

    return (
        "You are an expert Italian tutor designing CONJUGATION DRILLS.\n"
        "For EACH numbered item, write a short natural English sentence "
        "(5-10 words) whose Italian translation REQUIRES exactly that "
        "tense and person, plus that Italian translation.\n\n"
        "ITEMS:\n" + target_block + "\n\n"
        "HARD CONSTRAINTS for every item:\n"
        "1. The English must force the target tense (congiuntivo: after "
        '"I think/doubt that..."; condizionale: "would..."; imperfetto: '
        '"was ...-ing"/"used to"; passato prossimo: a completed action; '
        "imperativo: a command).\n"
        "2. The English subject must match the person: io=I, tu=you(sg), "
        "lui/lei=he/she or a singular noun, noi=we, voi=you all, "
        "loro=they or a plural noun.\n"
        "3. The Italian verb must be in exactly that tense AND person, and "
        "ALL adjectives and participles must agree in number and gender. "
        "For noi/voi/loro NEVER end an adjective or participle in singular "
        "-o/-a — use -i (m.pl.) or -e (f.pl.). "
        '("They are tired" = "Sono stanchi", NOT "Sono stanco".)\n'
        "4. Compound tenses: correct auxiliary (avere vs essere) plus "
        "participle agreement. -ire verbs: mind -isco (finire) vs plain "
        "(dormire).\n"
        "5. english_prompt and italian must mean exactly the same thing — "
        "no detail present in one and missing from the other.\n\n"
        "Return ONLY this JSON object, one entry per item, in order:\n"
        '{"cards":[{"index":1,"english_prompt":"...","italian":"...",'
        '"expected_form":"...","form_explanation":"...",'
        '"word_breakdown":[{"italian":"...","english":"...","note":"..."}],'
        '"grammar_point":{"structure":"...","explanation":"..."}}]}'
    )


def generate_conjugation_batch(word_dicts):
    """
    Generate conjugation drills for many verbs in ONE API call.
    Returns a list aligned to word_dicts, None where unusable. Never raises.
    """
    results = [None] * len(word_dicts)
    if not word_dicts:
        return results

    # Choose a (tense, person) target per verb up front.
    specs = []
    for w in word_dicts:
        tense, person = _pick_conjugation_target()
        specs.append((
            (w.get("italian") or "").strip(),
            w.get("english", ""),
            tense,
            person,
        ))

    for chunk_start in range(0, len(specs), BATCH_MAX_ITEMS):
        chunk_specs = specs[chunk_start:chunk_start + BATCH_MAX_ITEMS]
        prompt = _build_conjugation_batch_prompt(chunk_specs)
        ...
        for attempt in range(2):
            try:
                ...
                category, desc = _classify_llm_error(e)
                logging.warning(
                    f"[batch] Conjugation batch attempt {attempt + 1}/2 failed "
                    f"[{category}]: {desc}"
                )
                _record_last_error(category)
                if category in ("auth", "rate_limit", "model_gone"):
        ...
        for local_i, (inf, eng, tense, person) in enumerate(chunk_specs):
            raw_item = by_index.get(local_i + 1)
            if raw_item is None:
                continue
            try:
                word_breakdown = []
                for wb_item in (raw_item.get("word_breakdown") or []):
                    if not isinstance(wb_item, dict):
                        continue
                    wb = {
                        "italian": wb_item.get("italian", ""),
                        "english": wb_item.get("english", ""),
                        "note": wb_item.get("note", ""),
                    }
                    if wb_item.get("stress"):
                        wb["stress"] = wb_item["stress"]
                    word_breakdown.append(wb)

                english_prompt = (raw_item.get("english_prompt") or "").strip()
                italian = (raw_item.get("italian") or "").strip()
                if not english_prompt or not italian:
                    continue
                ...
                candidate = {
                    ...
                    "italian": italian,
                    "english_correct": english_prompt,
                    "expected_form": raw_item.get("expected_form", ""),
                    "form_explanation": raw_item.get("form_explanation", ""),
                    "word_breakdown": word_breakdown,
                    "grammar_point": raw_item.get("grammar_point", {}) or {},
                    "expression_note": {},
                    "english_distractors": [],
                    "target_category": "verb_form",
                }

                # Same agreement verifier as the single-card path.
                is_ok, issue = _verify_conjugation_drill(candidate, tense, person)
                if not is_ok:
                    logging.warning(
                        f"[batch] Conjugation item {local_i + 1} failed verification "
                        f"({issue}) — dropped, will regenerate individually."
                    )
                    continue

                results[chunk_start + local_i] = candidate
            except Exception as e:
                logging.warning(f"[batch] Could not finalise conjugation item: {e}")
                continue

    produced = sum(1 for r in results if r is not None)
    logging.info(f"[batch] Conjugation batch produced {produced}/{len(word_dicts)} drills.")
    if produced:
        _record_last_error(None)
    return results


def _apply_cloze_to_base(base, target_word):
    """
    Turn a finished dictation card into a cloze card by blanking the target
    word. Mirrors generate_cloze_exercise's logic. Falls back to free
    recall when the target isn't present verbatim.
    """
    import re as _re
    sentence = base.get("italian", "")
    if not target_word:
    ...
    if not match:
        base["exercise_type"] = "free"
        return base
    base["exercise_type"] = "cloze"
    base["cloze_blank"] = match.group(0)
    base["cloze_display"] = sentence[:match.start()] + "___" + sentence[match.end():]
    return base


def prefetch_session_exercises(words, modes):
    """
    Generate a whole session's exercises in as few API calls as possible.

    Returns {index_as_str: exercise_dict} covering however many cards could
    be produced. Any index missing from the result is simply generated
    on demand later by the existing per-card path, so a partial or empty
    ...
    out = {}
    try:
        if not words or not modes or len(words) != len(modes):
            return out

        dictation_idx, dictation_words = [], []   # listening + free recall
        cloze_idx, cloze_words = [], []           # dictation + local blanking
        conj_idx, conj_words = [], []             # conjugation drills

        for i, (word, mode) in enumerate(zip(words, modes)):
            if mode == "listen":
                dictation_idx.append(i)
                dictation_words.append(word)
                continue
            try:
                strategy = choose_recall_strategy(word)
            except Exception:
                strategy = "free"
        ...
        # --- One call (per chunk) for every dictation-family card ---
        combined_words = dictation_words + cloze_words
        combined_idx = dictation_idx + cloze_idx
        if combined_words:
            produced = generate_dictation_batch(combined_words)
            n_plain = len(dictation_words)
            for slot, card in enumerate(produced):
                if card is None:
                    continue
                session_i = combined_idx[slot]
                if slot < n_plain:
                    # listening keeps exercise_type "listen"; recall becomes free
                    if modes[session_i] != "listen":
                        card["exercise_type"] = "free"
                else:
                    target = (combined_words[slot].get("italian") or "").strip()
                    card = _apply_cloze_to_base(card, target)
                out[str(session_i)] = card

        # --- One call (per chunk) for every conjugation drill ---
        if conj_words:
            produced = generate_conjugation_batch(conj_words)
            for slot, card in enumerate(produced):
                if card is not None:
                    out[str(conj_idx[slot])] = card

        logging.info(
            f"[batch] Prefetched {len(out)}/{len(words)} cards for this session."
        )
    except Exception as e:
        logging.warning(f"[batch] Prefetch failed entirely ({e}); "
                        f"falling back to per-card generation.")
        return {}
    return out
